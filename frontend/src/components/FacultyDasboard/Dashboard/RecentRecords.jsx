...
export default function RecentRecords() {
  ...
  useEffect(() => {
    const fetchRecentAttendance = async () => {
      try {
        setLoading(true);
        const response = await getRecentAttendance(5); // Fetch 5 recent records
        ...
        console.log("Attendance records received:", attendanceData);
  ...
  const getSubjectName = (subject) => {
    ...
    // Log the subject structure to help debug
    console.log("Subject structure:", subject);
    
    if (typeof subject === 'object' && subject.subjectName) {
      return subject.subjectName;
    } else if (typeof subject === 'string') {
      return 'Subject ID: ' + subject.substring(0, 8) + '...'; // Show truncated ID
    } else {
  ...
  if (records.length === 0) {
    return (
      <div className="mt-8 md:mt-10 bg-white rounded-xl shadow-md p-6">
        <h2 className="text-lg md:text-xl font-semibold mb-4">Recent Attendance Records</h2>
        <p className="text-gray-600 text-center py-6">No attendance records found.</p>
      </div>
    );
  }
          ...
          <table className="w-full min-w-[800px]">
            ...
            <tbody className="divide-y divide-gray-200">
              {records.map((record, index) => {
                ...
                const totalStudents = record.students?.length || record.totalStudents || 0;
                ...
                return (
                  <tr key={record._id || index} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 md:px-6 py-4 text-sm">{formatDate(record.date)}</td>
                    <td className="px-4 md:px-6 py-4 text-sm">{totalStudents}</td>
                    <td className="px-4 md:px-6 py-4 text-sm">
